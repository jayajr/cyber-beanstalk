...
class SecretsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create secret" do
    assert_difference('Secret.count') do
      post secrets_url, params: { secret: { name: @secret.name, secret: @secret.secret, uuid: @secret.uuid } }
    end
  ...
  test "should update secret" do
    patch secret_url(@secret), params: { secret: { name: @secret.name, secret: @secret.secret, uuid: @secret.uuid } }
